...
// Verify the request method
    ...
    try {   
        ...
        $advancedMedia->filters()
            ...
            //creditos
            ->custom('[bgcred]',"drawtext=enable='between(t,0,15)':fontfile=/path/to/font.ttf:text='Créditos\n\n".$_POST['creditos'].$CREDITOS."':".
        ...
        if ($_POST['creditos']=="animation")  {
            $format->setAdditionalParameters(['-crf','28','-tune','animation']);
        }
        else if ($_POST['creditos']=="film") { 
            $format->setAdditionalParameters(['-crf','28','-tune','film']);
        }
        else if ($_POST['creditos']=="test") { 
            $format->setAdditionalParameters(['-crf','28']);
        }
        else { 
            $format->setAdditionalParameters(['-crf','28','-tune','stillimage']);
        }        
        ...
        $format->on('progress', function ($video, $format, $percentage) {
            ...
            $advancedMedia
                ->map(array('[outv]','[outa]'), $format, $APP.'/output/'.$_FILES["arquivoVideo"]["name"])
                ->save();
